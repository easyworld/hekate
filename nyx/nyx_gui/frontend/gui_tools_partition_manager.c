...
static lv_res_t _action_flash_linux_data(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map2[] = { "\223刪除安裝檔案", "\221確定", "" };
	...
	if (res)
	{
		lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 開啟第1部分失敗!");
	...
	while (total_size_sct > 0)
	{
		...
		if (bytesWritten >= fileSize)
		{
			...
			if (res)
			{
				s_printf(txt_buf, "#FFDD00 錯誤:# 開啟第%d部分失敗#", currPartIdx);
		...
		if (res)
		{
			lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 從SD卡讀取!");
			manual_system_maintenance(true);
		...
		while (res)
		{
			...
			if (retryCount >= 3)
			{
				lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 往SD卡寫入!");
				manual_system_maintenance(true);
...
static lv_res_t _action_check_flash_linux(lv_obj_t *btn)
{
	...
	if (f_stat(path, NULL))
	{
		lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 未找到安裝檔案!");
		goto error;
	}
	...
	while (true)
	{
		...
		if ((u64)fno.fsize % SZ_4M)
		{
			...
			{
				lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 映象沒有4MiB對齊!");
				goto error;
			}
...
static lv_res_t _action_flash_android_data(lv_obj_t * btns, const char * txt)
{
	...
	if (f_stat(path, NULL))
	{
		s_printf(txt_buf, "#FF8000 警告:# 未找到核心映象!\n");
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			s_printf(txt_buf, "#FF8000 警告:# 核心映象太大!\n");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);

			s_printf(txt_buf, "#C7EA46 成功:# 核心映象已刷入!\n");
			f_unlink(path);
		}
...
boot_img_not_found:
	...
	if (f_stat(path, NULL))
	{
		...
		if (f_stat(path, NULL))
		{
			strcat(txt_buf, "#FF8000 警告:# TWRP映象未找到!\n");
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 警告:# 恢復映象太大!\n");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);
			strcat(txt_buf, "#C7EA46 成功:# 恢復映象已刷入!\n");
			f_unlink(path);
		}
...
recovery_not_found:
	...
	if (f_stat(path, NULL))
	{
		...
		if (f_stat(path, NULL))
		{
			strcat(txt_buf, "#FF8000 警告:# DTB映象未找到!");
			goto dtb_not_found;
		}
	}
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 警告:# DTB映象太大!");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);
			strcat(txt_buf, "#C7EA46 成功:# DTB映象已刷入!");
			f_unlink(path);
		}
...
static lv_res_t _sd_create_mbox_start_partitioning()
{
	...
	lv_label_set_text(lbl_status, "#00DDFF 狀態:# 備份檔案中...");
	manual_system_maintenance(true);
	...
	{
		if (part_info.backup_possible)
			lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 備份檔案失敗!");
		else
			lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 備份檔案失敗!\nBootloader資料夾超過1.2GB或被汙染!");
	...
	if (mkfs_error)
	{
		...
		lv_label_set_text(lbl_status, "#00DDFF 狀態:# 恢復檔案中...");
		manual_system_maintenance(true);
		...
		if (_backup_and_restore_files(false, lbl_paths))
		{
			...
			if (_backup_and_restore_files(false, lbl_paths))
			{
				lv_label_set_text(lbl_status, "#FFDD00 錯誤:# 恢復檔案失敗!");
				free(buf);
				goto error;
			}
		}

		lv_label_set_text(lbl_status, "#00DDFF 狀態:# 檔案已恢復但操作失敗!");
		f_unmount("ram:");
		free(buf);
		goto error;
	}
...
static lv_res_t _emmc_create_mbox_start_partitioning()
{
	...
	static const char *mbox_btn_map1[] = { "\251", "\222刷寫 Linux", "\222刷寫 Android", "\221OK", "" };
	static const char *mbox_btn_map2[] = { "\251", "\222刷寫 Linux", "\221OK", "" };
	static const char *mbox_btn_map3[] = { "\251", "\222刷寫 Android", "\221OK", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	lv_label_set_text(lbl_status, "#00DDFF 狀態:# 初始化中...");
...
static void _create_mbox_check_files_total_size()
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222確定", "\251", "" };
	static const char *mbox_btn_map2[] = { "\222不備份", "\222確定", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
...
check_changes:
	...
	{
		s_printf(txt_buf, "#00DDFF 以下需要更正:#\n");
		if (has_mbr_attributes)
...
lv_res_t create_window_partition_manager(bool emmc)
{
	...
	else
	{
		if (!emmc_initialize(false))
		{
			lv_obj_t *lbl = lv_label_create(h1, NULL);
			lv_label_set_recolor(lbl, true);
			lv_label_set_text(lbl, "#FFDD00 初始化eMMC失敗!#");
			return LV_RES_OK;
		}
		emmc_set_partition(EMMC_GPP);
	}
	...
	lv_label_set_static_text(lbl_hos, !emmc ? "#96FF00 "SYMBOL_DOT" 官方系統(FAT32):#" :
	...
	lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷寫Linux");
	if (!emmc)
	...
	case 0: // Disable Flash Android button if partition not found.
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷寫Android");
		lv_obj_set_click(btn_flash_android, false);
		lv_btn_set_state(btn_flash_android, LV_BTN_STATE_INA);
		break;

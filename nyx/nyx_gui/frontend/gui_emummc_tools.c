...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_standard_window(SYMBOL_DRIVE"  建立 SD檔案emuMMC", _action_emummc_window_close);
	else
		win = nyx_create_standard_window(SYMBOL_DRIVE"  建立 SD分割區emuMMC", _action_emummc_window_close);
...
static void _create_mbox_emummc_raw()
{
	...
	if (mbr_ctx.available)
	{
		...
		s_printf(txt_buf, "#FFDD00 找不到適用的分割區!#\n\n");

	s_printf(txt_buf + strlen(txt_buf),
		"分割區表:\n"
		"#C0C0C0 第0部分: 類型: %02x, 起始: %08x, 大小: %08x#\n"
		"#%s 第1部分: 類型: %02x, 起始: %08x, 大小: %08x#\n"
		"#%s 第2部分: 類型: %02x, 起始: %08x, 大小: %08x#\n"
		"#%s 第3部分: 類型: %02x, 起始: %08x, 大小: %08x#",
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 注意:# 黃色條目表示使用者分割區已被調整大小.");

	if (!mbr_ctx.available)
		strcat(txt_buf, "\n#FF8000 您想要對SD卡進行分割區嗎?#\n"
						  "#FF8000  (您將被要求選擇如何繼續) #");
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222SD卡檔案", "\222SD卡分割區", "\222取消", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	lv_mbox_set_text(mbox,
		...
		"請選擇要建立的emuMMC類型.\n"
		"#FF8000 SD卡檔案# 以檔案形式儲存在FAT分割區中.\n"
		"#FF8000 SD卡分割區# 以原始映像方式儲存在可用分割區中.");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map1[] = { "\251", "確定", "\251", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	{
		s_printf(txt_buf,
			"#C7EA46 找到以SD分割區為基礎的emuMMC!#\n\n"
			"#FF8000 你要修復它的設定與分割區類型嗎?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig0_action);
	}
	else
	{
		s_printf(txt_buf, "未偵測到 emuMMC!\n");
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262備份", "\262修復 RAW", "\222取消", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  變更emuMMC", _action_win_change_emummc_close);
...
out0:;
	...
	lv_label_set_static_text(label_txt, "SD卡分割區");
	...
	for (u32 raw_btn_idx = 0; raw_btn_idx < 3; raw_btn_idx++)
	{
		...
		{
			s_printf(txt_buf, "SD卡原始分割區%d", raw_btn_idx + 1);
...
lv_res_t create_win_emummc_tools(lv_obj_t *btn)
{
	...
	{
		lv_label_set_static_text(label_txt2, "emuMMC已停用, 開機將使用eMMC.\n\n\n");
	}
	...
	lv_label_set_static_text(label_btn, SYMBOL_SETTINGS"  變更emuMMC");
	...
	lv_label_set_static_text(label_txt2,
		"選擇在emuMMC資料夾或SD卡分割區中建立系統映象.\n"
		"您最多可以擁有3個基於分割區的系統映象和無數個基於檔案的系統映象.\n");
	...
	lv_label_set_static_text(label_txt4,
		"可建立新的 #C7EA46 SD檔案# 或 #C7EA46 SD Raw 分割區#\n"
		"emuMMC. 您可透過 eMMC 或 eMMC 備份來建立.\n");
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"將備份移轉至 #C7EA46 SD檔案 emuMMC#, 或修復現有\n#C7EA46 SD Raw 分割區 emuMMC#.\n");
	lv_obj_set_style(label_txt4, &hint_small_style);
